...
const createProductSchema = z.object({
  ...
  category_id: z.number().positive("Category ID must be positive").optional(),
...
const createCategorySchema = z.object({
  name: z.string().min(1, "Category name is required").max(120, "Category name too long"),
});
...
export async function getAllProductsAdmin(req: AuthenticatedRequest, res: Response) {
  try {
...
export async function updateCategory(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    const { rows } = await pool.query(updateQuery, [name, categoryId]);
